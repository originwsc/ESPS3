...
void lvgl_demo(void)
{
    if (images_init())    /* 初始化图片库 */
    {
        if (sd_spi_init() == ESP_OK)   /* 检测到SD卡，更新图片库 */
        {
            sd_check_en = 1;
        ...
        {
            printf("SD Card not found, skip image library update.\r\n");
            lcd_show_string(30, 50, 200, 16, 16, "SD Card Not Found!", RED);
            vTaskDelay(1000);
            lcd_clear(WHITE);
        }
    }
